      ...
      <div className="shrink-0 border-t border-border px-3 py-2.5">
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            className="flex-1 rounded-md border border-border bg-bg px-3 py-2 text-sm text-fg placeholder:text-fg-muted focus:outline-none focus:ring-2 focus:ring-primary disabled:opacity-50 min-h-[44px]"
          />
          <button
            onClick={handleSubmit}
            disabled={isThrottled || !input.trim() || input.trim().length > MAX_MESSAGE_LENGTH}
            className="shrink-0 rounded-md bg-primary px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer min-h-[44px]"
          >
            Send
          </button>
        </div>
